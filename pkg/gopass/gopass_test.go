	...
	for _, masked := range []bool{true, false} {
		for _, d := range ds {
			...
			if string(result) != d.password {
				t.Errorf("Expected %q but got %q instead when masked=%v. %s",
					d.password, result, masked, d.reason)
			}
			if leftOnBuffer != d.bytesLeft {
				t.Errorf("Expected %v bytes left on buffer but instead got %v when masked=%v. %s",
					d.bytesLeft, leftOnBuffer, masked, d.reason)
			}
		}
	}
}
...
func TestMaxPasswordLength(t *testing.T) {
	type testData struct {
		...
		// Helper field to output in case of failure; rather than hundreds of bytes.
		inputDesc string
	}
	...
	for _, d := range ds {
		...
		if err != d.expectedErr {
			t.Errorf("Expected error to be %v; instead got %v from %v", d.expectedErr, err, d.inputDesc)
		}
	}
}
