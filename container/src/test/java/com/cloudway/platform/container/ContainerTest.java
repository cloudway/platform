...
public class ContainerTest
{
    ...
    private final PluginControl plugins = context.mock(PluginControl.class);
    ...
    private ContainerAdapter makeFakeContainerAdapter(Container c) {
        return new FakeContainerAdapter(c, adapter, userDatabase, filePermissions);
    }

    private static DefaultContainer createContainer(String id) throws IOException {
        return DefaultContainer.create(id, "mock", "demo", "small");
    }

    private static DefaultContainer createContainer() throws IOException {
        return createContainer(makeUUID());
    }

    private Container createMockContainer(String id) throws IOException {
        DefaultContainer ac = createContainer(id);
        ac.setPluginControl(plugins);
        return ac;
    }

    private Container createMockContainer() throws IOException {
    ...
    public void load_container_from_id() throws IOException {
        ...
        Container ac = Container.fromId(id);
    ...
    private void checkContainer(Container ac) {
    ...
    public void create_application_container() throws IOException {
        String id = makeUUID();
        Container ac = createContainer(id);
    ...
    public void destroy_application_container() throws IOException {
        String id = makeUUID();
        Container ac = createMockContainer(id);
        Path home = ac.getHomeDir();

        context.checking(new Expectations() {{
            oneOf(plugins).destroy();
            oneOf(adapter).destroy();
        }});

        ac.destroy();

        assertFalse(Container.exists(id));
        assertFalse(userDatabase.getpwname(id) != null);
        assertFalse(Files.exists(home));
    }

    private static void checkEnvVar(Container ac, String key, String value)
        throws IOException
    {
        if (value != null) {
    ...
    private static void failedToCreateContainer(String id, String name, String namespace) throws IOException {
        try {
            Container.create(id, name, namespace, "small");
    ...
    public void add_authorized_key() throws IOException {
        ...
        Container ac = createMockContainer();
    ...
    private class ContainerStateMachine {
        ...
        public ContainerStateMachine(Container container) {
        ...
        private void allow_plugins_start() throws IOException {
            context.checking(new Expectations() {{
                oneOf(plugins).start();
            }});
        }

        private void allow_plugins_stop() throws IOException {
            context.checking(new Expectations() {{
                oneOf(plugins).stop();
            }});
        }

        private void allow_plugins_restart() throws IOException {
            context.checking(new Expectations() {{
                oneOf(plugins).restart();
            }});
        }
        ...
        public ContainerStateMachine tidy() throws IOException {
            ...
            context.checking(new Expectations() {{
                if (state == STARTED) {
                    States state = context.states("plugins").startsAs("init");
                    oneOf(plugins).stop();
                        when(state.is("init"));
    ...
    public void install_plugin_with_empty_repo() throws Exception {
        ...
        installPluginWithRepo(container, source, "empty");
    }
    ...
    public void install_plugin_with_template_repo() throws Exception {
        ...
        installPluginWithRepo(container, source, null);
    }

    @Test
    public void install_plugin_with_external_repo() throws Exception {
        Container container = createContainer();
        Path source = preparePluginDir(withNoContents());
        ...
        installPluginWithRepo(container, source, repo);
    }
    ...
    public void process_template() throws Exception {
        ...
        String ip = installPluginWithRepo(container, source, "empty");
    ...
    private String installPluginWithRepo(Container container, Path source, String repo)
        throws IOException
    {
        ...
        List<Plugin.Endpoint> eps = container.getEndpoints();
        assertThat(eps.size(), is(1));
        Plugin.Endpoint ep = eps.get(0);
    ...
    private Path preparePluginDir(Map<String, String> files) throws Exception{
        return prepareDirectory(temporary.newFolder(), withContents()
            .put("metadata/plugin.xml", createPluginXml())
    ...
    private static String createPluginXml() throws JAXBException {
        MetaData.Plugin metadata = new MetaData.Plugin() {{
            name          = "mock";
            displayName   = "Mock";
            version       = "1.0";
            vendor        = "cloudway";
            pluginVersion = "1.0";
            pluginVendor  = "cloudway";
