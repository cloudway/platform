...
(define-macro (define-condition-type name supertype predicate . fields)
  (with-gensyms (thing condition)
    `(begin
       (define ,name
...
(define-macro (condition . type-field-plist)
  `(type-field-alist->condition
    (list
      ,@(map (case-lambda
               (((type . fields))
                 `(cons ,type
                        (list ,@(map (case-lambda
                                       (((field value))
                                        `(cons ',field ,value)))
                                     fields)))))
             type-field-plist))))
  ...
  (let loop ((type-field-alist the-type-field-alist))
    (if (not (null? type-field-alist))
        (let* ((entry (car type-field-alist))
          ...
          (for-each (lambda (missing-fields)
                      (let ((supertype (condition-type-field-supertype type missing-field)))
