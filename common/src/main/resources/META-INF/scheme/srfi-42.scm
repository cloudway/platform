...
(define-macro (do-ec . form)
  (match form
    ...
    [((':do olet lhs ne1? ilet ne2? lss) cmd)
     `(do-ec:do ,cmd (:do ,olet ,lhs ,ne1? ,ilet ,ne2? ,lss))]
...
; one- and two- sided if
    ...
    [(('if #f consequent))
     (void)]
    [(('if #t consequent alternate))
     consequent]
    [(('if #f consequent alternate))
     alternate]

    ; (not (not <test>))
    [(('if ('not ('not test)) consequent))
     `(ec-simplify (if ,test ,consequent))]
    [(('if ('not ('not test)) consequent alternate))
    ...
    ; begin, flatten use helper (ec-simplify 1 done to-do)
    [(('begin . command))
...
(define-macro (:parallel-1 . form)
  (match form
    ; process next element of to-do, reentry at (**)
    [(cc ((g arg1 . arg) . gen) result)
...
(define-macro (:while cc (g arg1 . arg) test)
...
(define-macro (:while-1 cc test (':do olet lbs ne1? ilet ne2? lss))
  `(:while-2 ,cc ,test () () () (:do ,olet ,lbs ,ne1? ,ilet ,ne2? ,lss)))

(define-macro (:while-2 . form)
  (with-gensyms (ib-tmp ne1?-value)
    (match form
      [(cc
       test
       ib-let
...
(define-macro (:until cc (g arg1 . arg) test)
  `(,g (:until-1 ,cc ,test) ,arg1 ,@arg))

(define-macro (:until-1 cc test (':do olet lbs ne1? ilet ne2? lss))
...
(define-macro (:string . form)
  (match form
    [(cc var ('index i) arg)
     (with-gensyms (str len)
       `(:do ,cc
             (let ((,str ,arg) (,len 0))
...
(define-macro (:vector . form)
  (match form
    [(cc var arg)
     (with-gensyms (i)
       `(:vector ,cc ,var (index ,i) ,arg))]
    [(cc var ('index i) arg)
     (with-gensyms (vec len)
       `(:do ,cc
             (let ((,vec ,arg) (,len 0))
...
(define-macro (:integers cc var . index)
  (match index
    (() `(:do ,cc ((,var 0)) #t ((+ ,var 1))))
    (('index i) `(:do ,cc ((,var 0) (,i 0)) #t ((+ ,var 1) (+ ,i 1))))))


(define-macro (:range . form)
  (match form
    ...
    ; the general case
    [(cc var arg1 arg2 arg3)
     (with-gensyms (a b s stop)
       `(:do ,cc
...
(define-macro (:real-range . form)
  (match form
    ...
    [(cc var ('index i) arg1 arg2 arg3)
     (with-gensyms (a b s istop)
       `(:do ,cc
...
(define-macro (:dispatched . form)
  (match form
    [(cc var ('index i) dispatch arg1 . arg)
     `(:parallel ,cc
                 (:integers ,i)
                 (:dispatched ,var ,dispatch ,arg1 ,@arg))]
    [(cc var dispatch arg1 . arg)
     (with-gensyms (d args g empty)
       `(:do ,cc
             (let ((,d ,dispatch)
                   (,args (list ,arg1 ,@arg))
                   (,g #f)
...
(define-macro (:generator-proc . form)
  (match form
    ...
    [(var
      (':do ('let obs . oc)
            ...
            ne2?
            ls))
     (with-gensyms (ne2 empty value)
       `(ec-simplify
          (let ,obs
            ,@oc
            (let (,@lbs (,ne2 #t))
              (ec-simplify
                (let (,@(map (lambda (x) (list (car x) #f)) ibs))
                  (lambda (,empty)
                    (if (and ,ne1? ,ne2)
                        (ec-simplify
                          (begin
                            ,@(map (lambda (x) `(set! ,@x)) ibs)
                            ,@ic
                            (let ((,value ,var))
                              (ec-simplify
                                (if ,ne2?
                                    (ec-simplify
                                      (begin ,@(map (lambda (v s) `(set! ,(car v) ,s))
                                                    lbs ls)))
                                    (set! ,ne2 #f)))
                              ,value)))
                        ,empty))))))))]
...
(define-macro (fold3-ec . form)
  (match form
    [(x0 ('nested . q1) q etc1 etc2 etc3 . etc)
     `(fold3-ec ,x0 (nested ,@q1 q) ,etc1 ,etc2 ,etc3 ,@etc)]
    [(x0 q1 q2 etc1 etc2 etc3 . etc)
     `(fold3-ec ,x0 (nested ,q1 ,q2) ,etc1 ,etc2 ,@etc)]
    [(x0 expression f1 f2)
     `(fold3-ec ,x0 (nested) ,expression ,f1 ,f2)]

    [(x0 qualifier expression f1 f2)
     (with-gensyms (empty result value)
...
(define-macro (append-ec etc1 . etc)
  `(apply append (list-ec ,etc1 ,@etc)))

(define-macro (string-ec etc1 . etc)
...
(define-macro (string-append-ec etc1 . etc)
  `(apply string-append (list-ec ,etc1 ,@etc)))

(define-macro (vector-ec etc1 . etc)
...
(define-macro (vector-of-length-ec . form)
  (match form
    [(k ('nested . q1) q etc1 . etc)
    ...
    [(k qualifier expression)
     (with-gensyms (len vec i)
       `(let ((,len ,k))
          (let ((,vec (make-vector ,len))
            ...
            (if (= ,i ,len)
                ,vec
                (error "vector is too long for comprehension")))))]))


(define-macro (sum-ec etc1 . etc)
  `(fold-ec (+) ,etc1 ,@etc +))

(define-macro (product-ec etc1 . etc)
  `(fold-ec (*) ,etc1 ,@etc *))

(define-macro (min-ec etc1 . etc)
  `(fold3-ec (min) ,etc1 ,@etc min min))

(define-macro (max-ec etc1 . etc)
  `(fold3-ec (max) ,etc1 ,@etc max max))

(define-macro (last-ec . form)
  (match form
    [(default ('nested . q1) q etc1 . etc)
     `(last-ec ,default (nested ,@q1 ,q) ,etc1 ,@etc)]
    [(default q1 q2 etc1 . etc)
     `(last-ec ,default (nested ,q1 ,q2) ,etc1 ,@etc)]
    [(default expression)
     `(last-ec ,default (nested) ,expression)]

    [(default qualifier expression)
     (with-gensyms (result)
       `(let ((,result ,default))
